...
exports.redeemLicenseCode = async (req, res) => {
    try {
        ...
        if (!vendorResponse) {
            ...
            } else if (code.startsWith('VEND-') || code.startsWith('SHEBA-') || code.length >= 6) {
                // Activation Code / Standard Enterprise License Key (e.g. VEND-5KQS-BC4V-EXKM-CVBR) - Stack on existing expiry
...
// API: Activate / Register 15-Day Free Trial
exports.activateTrial = async (req, res) => {
    try {
        ...
        // Forward to central Vendor API if reachable
        if (vendorUrl) {
            try {
                const controller = new AbortController();
                const timeoutId = setTimeout(() => controller.abort(), 4000);
...
module.exports = {
    ...
    activateTrial: exports.activateTrial,
};
