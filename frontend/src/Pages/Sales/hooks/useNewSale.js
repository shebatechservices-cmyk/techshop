...
export function useNewSale({
  ...
  const addProduct = (prod, initialSerial = '', forcedUnit = null) => {
    ...
    const activeUnitName = isSubUnitScan ? (prod.sub_unit_name || 'Meter') : (prod.unit_name || 'Pcs');

    const existing = items.find((it) => it.product_id === prod.id && it.unit_type === currentUnitType);
    if (existing) {
      const newSerials =
    ...
    const line = {
      ...
      cost_price: isSubUnitScan && conversionRate > 1 ? Number((unitCost / conversionRate).toFixed(2)) : unitCost,
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (found && Number(found.stock || 0) > 0) {
          const isSub = found.sub_unit_barcode && found.sub_unit_barcode.toLowerCase() === lowerQ;
