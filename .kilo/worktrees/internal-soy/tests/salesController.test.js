...
describe('updateSale', () => {
    ...
    test('rejects edits older than 7 days (168 hours)', async () => {
        setupClient({
            'SELECT \\* FROM sales WHERE': () => ({ rows: [{ ...existingSale(), created_at: new Date(Date.now() - 169 * 3600 * 1000) }] }),
        });
        const res = mockRes();
...
describe('deleteSale', () => {
    ...
    test('blocks deletion beyond the 72h window', async () => {
        const client = setupClient({
            'SELECT \\* FROM sales WHERE': () => ({ rows: [{ ...deletableSale(), created_at: new Date(Date.now() - 73 * 3600 * 1000) }] }),
        });
        const res = mockRes();
        await sales.deleteSale({ params: { id: 77 } }, res);
        ...
        expect(res.json).toHaveBeenCalledWith(expect.objectContaining({
            success: false,
            message: expect.stringContaining('72 hours'),
        }));
        expect(client.query).not.toHaveBeenCalledWith('COMMIT');
    });

    test('blocks deletion when warranty claims exist (lock chain)', async () => {
        const client = setupClient({
            'SELECT \\* FROM sales WHERE': () => ({ rows: [deletableSale()] }),
