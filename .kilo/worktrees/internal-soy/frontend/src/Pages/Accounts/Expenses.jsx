...
const DEFAULT_CATEGORIES = [
  { id: 1, name: 'Tea & Entertainment (চা ও আপ্যায়ন)', icon: '☕', color: '#b45309', bg: '#fef3c7' },
  { id: 2, name: 'Technician Conveyance / TADA (যাতায়াত)', icon: '🛵', color: '#0284c7', bg: '#e0f2fe' },
  { id: 3, name: 'Shop & Warehouse Rent (দোকান ভাড়া)', icon: '🏢', color: '#7c3aed', bg: '#f3e8ff' },
  { id: 4, name: 'Electricity & Internet (বিদ্যুৎ ও ওয়াইফাই বিল)', icon: '⚡', color: '#d97706', bg: '#fffbeb' },
  { id: 5, name: 'Packaging & Printing (প্যাকেজিং ও স্টিকার)', icon: '📦', color: '#059669', bg: '#ecfdf5' },
  { id: 6, name: 'Staff Salaries & Allowance (বেতন ও অগ্রিম)', icon: '👥', color: '#dc2626', bg: '#fef2f2' },
  { id: 7, name: 'Shop Tools & Maintenance (মেরামত ও টুলস)', icon: '🛠️', color: '#475569', bg: '#f1f5f9' },
  { id: 8, name: 'Others / Miscellaneous (বিবিধ খরচ)', icon: '📌', color: '#334155', bg: '#f8fafc' },
];
...
export default function Expenses() {
  ...
  const [newExpense, setNewExpense] = useState({
    category_name: 'Tea & Entertainment (চা ও আপ্যায়ন)',
    account_id: '',
  ...
  const loadData = async () => {
    try {
      ...
      const [expRes, catRes, accRes, ovRes] = await Promise.all([
        ...
        fetch(`${API}/expenses/categories`).catch(() => null),
  ...
  const handleCreateExpense = async (e) => {
    ...
    try {
      ...
      setNewExpense({
        category_name: categories[0]?.name || 'Tea & Entertainment (চা ও আপ্যায়ন)',
  ...
  const handleCreateCategory = async (e) => {
    ...
    try {
      ...
      const payload = { name: newCategoryName.trim(), description: newCategoryDesc.trim() };
      setNewCategoryName('');
      setNewCategoryDesc('');
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setNewExpense({ ...newExpense, category_name: e.target.value })}
            ...
            <form onSubmit={handleCreateCategory}>
              <div style={{ marginBottom: '12px' }}>
                ...
                <input
                  type="text"
                  required
                  placeholder="e.g. CCTV Testing Tools or Generator Fuel"
              ...
              <div style={{ marginBottom: '16px' }}>
                ...
                <input
                  type="text"
                  placeholder="Brief description of this expense head"
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px' }}>
                ...
                <button
                  type="submit"
                  style={{ padding: '7px 18px', borderRadius: '6px', border: 'none', background: '#0f172a', color: '#ffffff', fontSize: '0.82rem', fontWeight: 700, cursor: 'pointer' }}
                >
                  Save Category
                </button>
              </div>
