...
export default function QuickAddProductModal({
  ...
  const handleChange = (field, value) => {
    setFormData((prev) => {
      ...
      if (field === 'brand_name' || field === 'name') {
        if (!prev.sku || prev.sku.startsWith('GEN-') || prev.sku.startsWith('PRD-')) {
          updated.sku = generateSku(updated.brand_name || 'PRD', updated.name || 'ITM');
        }
      }
      return updated;
    });
  ...
  const handleSubmit = async (e) => {
    ...
    // 1. Duplicate check against existing products
    const isDup = existingProducts.some((p) => {
      const matchName = (p.name || '').trim().toLowerCase() === trimmedName.toLowerCase();
      const matchBrand = (p.brand_name || '').trim().toLowerCase() === (formData.brand_name || '').trim().toLowerCase();
      const matchModel = (p.model_name || '').trim().toLowerCase() === (formData.model_name || '').trim().toLowerCase();
      const matchSku = formData.sku && (p.sku || '').trim().toLowerCase() === formData.sku.trim().toLowerCase();
      const matchBarcode = formData.barcode && (p.barcode || '').trim() === formData.barcode.trim();

      return (matchName && matchBrand && matchModel) || matchSku || matchBarcode;
    });
