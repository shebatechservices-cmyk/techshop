...
export default function GlobalSearchBar({ onNavigate, compact = false }) {
  ...
  return (
    <div style={{ position: 'relative', width: compact ? '100%' : '100%', marginBottom: compact ? '0' : '18px', zIndex: 900 }}>
      {/* Search Input Bar */}
      <div
        style={{
          display: 'flex',
          ...
          border: isOpen ? '2px solid #0284c7' : compact ? '1px solid #94a3b8' : '1.5px solid #cbd5e1',
          padding: compact ? '3px 8px' : '6px 14px',
          boxShadow: isOpen
            ? '0 10px 25px -5px rgba(2, 132, 199, 0.18), 0 0 0 3px rgba(2, 132, 199, 0.1)'
            : compact ? '0 1px 3px rgba(0,0,0,0.12)' : '0 2px 5px rgba(0, 0, 0, 0.04)',
          transition: 'all 0.2s ease',
        }}
      >
        <span style={{ fontSize: compact ? '0.92rem' : '1.2rem', color: '#0284c7', marginRight: compact ? '5px' : '10px' }}>🔍</span>
        <input
          ref={inputRef}
          type="text"
          value={query}
          ...
          placeholder={compact ? "Search invoices, customers, suppliers..." : "Global Search: Invoices, Quotations, Customers, Suppliers..."}
          style={{
            flex: 1,
            minWidth: 0,
            border: 'none',
      ...
      {isOpen && query.trim().length > 0 && (
        <div
          ...
          style={{
            ...
            width: compact ? '520px' : '100%',
            maxWidth: '92vw',
            background: '#ffffff',
            borderRadius: '12px',
            border: '1px solid #cbd5e1',
            boxShadow: '0 20px 35px -5px rgba(0, 0, 0, 0.2), 0 10px 10px -5px rgba(0, 0, 0, 0.04)',
            maxHeight: '480px',
            overflowY: 'auto',
            zIndex: 99999,
            padding: '8px',
