...
export default function PurchaseLedgerPreviewModal({
  ...
  useEffect(() => {
    ...
    const targetId = orderId || (initialOrder && initialOrder.id);
    if (!targetId) return;
    ...
    const fetchDetails = async () => {
      ...
      setError('');
      try {
        let res = await fetch(`${API_BASE}/purchase/${targetId}`);
        if (!res.ok) {
          res = await fetch(`${API_BASE}/purchase/orders/${targetId}`);
        }
        if (res.ok) {
          const data = await res.json();
          setOrder(data);
        } else {
          setError('Failed to load purchase voucher details');
        }
      } catch (err) {
        console.error(err);
