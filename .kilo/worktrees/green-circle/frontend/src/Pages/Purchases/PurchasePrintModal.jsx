...
export default function PurchasePrintModal({
  ...
  const buildShareText = () => {
    let msg = `*${company.name.toUpperCase()} - PURCHASE ORDER RECEIPT*\n`;
    msg += `━━━━━━━━━━━━━━━━━━━━━\n`;
    msg += `*PO No:* ${poNumber}\n`;
    msg += `*Date:* ${dateStr}\n`;
    msg += `*Supplier:* ${supplierName}${supplierPhone ? ` (${supplierPhone})` : ''}\n`;
    if (order.transaction_reference) {
      msg += `*Reference:* ${order.transaction_reference}\n`;
    }
    msg += `-------------------------------------\n`;
    msg += `*PURCHASED ITEMS:*\n`;
    ...
    msg += `-------------------------------------\n`;
    msg += `*Items Subtotal:* ৳${itemsCost.toLocaleString()}\n`;
    if (extraCost > 0) {
      msg += `*Freight / Extra Cost:* ৳${extraCost.toLocaleString()}\n`;
    }
    msg += `*Current Order Total:* ৳${currentTotal.toLocaleString()}\n`;
    if (previousDue > 0) {
      msg += `*Previous Due:* ৳${previousDue.toLocaleString()}\n`;
    }
    msg += `*Total Payable:* ৳${totalPayable.toLocaleString()}\n`;
    msg += `*Amount Paid:* ৳${totalPaid.toLocaleString()}\n`;
    msg += `*Remaining Due:* ৳${remainingDue.toLocaleString()}\n`;
    msg += `━━━━━━━━━━━━━━━━━━━━━\n`;
    msg += `Thank you for partnering with ${company.name}!`;
    return msg;
  };
  ...
  const handleEmailShare = () => {
    const rawText = buildShareText();
    const subject = `Purchase Order ${poNumber} - ${company.name}`;
    const mailto = `mailto:${supplier?.email || ''}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(rawText)}`;
    ...
    <div className="print-modal-backdrop" style={{
      ...
      <div className="print-actions-bar" style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <div>
            <strong style={{ fontSize: '1rem', display: 'block' }}>Purchase Order A4 Print Preview</strong>
            <span style={{ fontSize: '0.78rem', color: '#94a3b8' }}>Order #{poNumber}</span>
          </div>
        </div>
                    ...
                    <tr style={{
                      ...
                      <td style={{ padding: '4px 8px' }}>
                        <div style={{ fontWeight: 700, color: '#0f172a', lineHeight: 1.3 }}>
                          {item.full_name || item.name || item.product_name || 'Product'}
                        </div>
