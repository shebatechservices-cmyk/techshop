...
const lazyWithRetry = (componentImport) =>
  lazy(async () => {
    try {
      ...
      // First retry dynamically after a small delay (smooths Vite HMR rebuild windows)
      try {
...
function App() {
  ...
  // Support ?login=1, ?logout=1, or ?dev=1 in URL
  useEffect(() => {
    try {
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      if (e.repeat) return;
      const isDKey = e.key === 'D' || e.key === 'd' || e.code === 'KeyD' || e.keyCode === 68;
  ...
  // Listen for session expiration events dispatched by fetch interceptor
  useEffect(() => {
    const handleExpired = () => {
      setCurrentUser(null);
      setShowLoginModal(true);
  ...
  useEffect(() => {
    ...
    // Verify immediately on login/mount
    verifySession();
  ...
  const handleFieldChange = (event) => {
    ...
    setForm((prev) => {
      ...
      if (name === 'isSerialRequired' || name === 'is_serial_required' || name === 'is_serial_tracked' || name === 'tracks_serial') {
  ...
  const handleEditProduct = (product) => {
    ...
    setForm({
      ...
      warranty_months: String(product.warranty_months !== undefined && product.warranty_months !== null ? product.warranty_months : 0),
    });
    setFeatureImageFile(null);
    setGalleryImageFiles([]);
    setFeatureImagePreview(null);
  ...
  return (
    <div className="app-frame" style={{ minHeight: '100vh', background: '#f8fafc', boxSizing: 'border-box' }}>
      <style>{`
        ...
        .catalog-row { display: grid; grid-template-columns: 50px 50px 2fr 1fr 1fr 1fr 110px 110px 100px 60px; align-items: center; padding: 12px 16px; border-bottom: 1px solid #f1f5f9; font-size: 0.88rem; color: #334155; }
            ...
            <div className="modal-card add-product-modal" role="dialog" aria-modal="true" aria-labelledby="add-product-title">
              ...
              <form onSubmit={handleSubmit} className="product-form">
                ...
                <div className="optional-fields" style={{ marginTop: '16px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  ...
                  <label className="optional-row" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: form.is_serial_required || form.isSerialRequired || form.tracks_serial ? '#ecfdf5' : '#f8fafc', padding: '10px 14px', borderRadius: '8px', border: form.is_serial_required || form.isSerialRequired || form.tracks_serial ? '1.5px solid #86efac' : '1px solid #e2e8f0', fontSize: '0.86rem', cursor: 'pointer' }}>
                    <span>
                      <strong style={{ color: form.is_serial_required || form.isSerialRequired || form.tracks_serial ? '#166534' : '#1e293b' }}>
                        📦 Requires Serial / Barcode Tracking
                      </strong>
                      <br />
                      <small style={{ color: '#64748b' }}>
                        Strictly enforced in Purchase & Sale forms. Automatically calculates quantity from scanned barcodes.
                      </small>
                    </span>
                    <input
                      type="checkbox"
                      ...
                      id="isSerialRequired"
                      checked={Boolean(form.is_serial_required || form.isSerialRequired || form.tracks_serial || form.is_serial_tracked)}
                      onChange={(event) => {
                  ...
                  <label className="optional-row" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: form.is_warranty_required || form.isWarrantyRequired ? '#eff6ff' : '#f8fafc', padding: '10px 14px', borderRadius: '8px', border: form.is_warranty_required || form.isWarrantyRequired ? '1.5px solid #93c5fd' : '1px solid #e2e8f0', fontSize: '0.86rem', cursor: 'pointer' }}>
                    <span>
                      <strong style={{ color: form.is_warranty_required || form.isWarrantyRequired ? '#1e40af' : '#1e293b' }}>
                        🛡️ Requires Warranty Tracking
                      </strong>
                      <br />
                      <small style={{ color: '#64748b' }}>
                        Enforces batch-specific warranty duration in Purchase entries and prints warranty on Customer Invoices.
                      </small>
                    </span>
                    <input
                      type="checkbox"
                      name="is_warranty_required"
                      id="isWarrantyRequired"
                      checked={Boolean(form.is_warranty_required || form.isWarrantyRequired || (Number(form.warranty_months || 0) > 0))}
                      onChange={(event) => {
                        const val = event.target.checked;
                        setForm((current) => ({
                          ...current,
                          ...
                          warranty_months: val ? (Number(current.warranty_months || 0) > 0 ? current.warranty_months : '12') : '0',
                        }));
                      }}
                  ...
                  {/* Warranty Months Input (Shown when warranty is enabled) */}
                  {(form.isWarrantyRequired || Number(form.warranty_months || 0) > 0) && (
                    <div style={{ padding: '10px 14px', background: '#eff6ff', borderRadius: '8px', border: '1px solid #bfdbfe', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px' }}>
                      <span style={{ fontSize: '0.84rem', fontWeight: 600, color: '#1e40af' }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <input
                          ...
                          placeholder="12"
                          style={{ width: '80px', padding: '6px 10px', borderRadius: '6px', border: '1px solid #93c5fd', fontSize: '0.86rem', textAlign: 'center', fontWeight: 700 }}
                        />
                        <span style={{ fontSize: '0.82rem', fontWeight: 600, color: '#475569' }}>Months</span>
                      </div>
                    </div>
                  )}

                  {[['is_service', 'Service Item (No Inventory)', 'Stock is not tracked for services'], ['is_bundle', 'Box / Bundle Product', 'Multiple products packaged together as a bundle']].map(([name, title, hint]) => (
                ...
                visibleProducts.map((product) => (
                  <div className="catalog-row" key={product.id}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px', overflow: 'hidden' }}>
                      <strong style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{productLabel(product) || product.name}</strong>
                    </div>
                    ...
                    <div>
                      {isProductSerialTracked(product) ? (
                        <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', fontSize: '0.75rem', fontWeight: 700, padding: '2px 8px', borderRadius: '4px', background: '#e0e7ff', color: '#3730a3', border: '1px solid #c7d2fe' }}>
                          ✅ Yes
                        </span>
                      ) : (
                        <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', fontSize: '0.75rem', fontWeight: 600, padding: '2px 8px', borderRadius: '4px', background: '#f1f5f9', color: '#64748b', border: '1px solid #e2e8f0' }}>
                          ❌ No
                        </span>
                      )}
                    </div>
                    <div>
                      {isProductWarrantyRequired(product) ? (
                        <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', fontSize: '0.75rem', fontWeight: 700, padding: '2px 8px', borderRadius: '4px', background: '#ecfdf5', color: '#065f46', border: '1px solid #a7f3d0' }}>
      ...
      {/* Vendor License Kill Switch Barrier Screen */}
      {licenseState?.is_blocked && (
        <LicenseLockScreen
          licenseInfo={licenseState}
          onReactivated={fetchLicenseCheck}
        />
      )}

      {/* Cash Register Shift Closing & Blind Close Modal */}
