...
export default function Purchases({ onOpenAddProduct, initialTab = 'history', initialSearch = '', navKey = 0 }) {
  ...
  const handleEditOrder = async (order) => {
    ...
    if (hoursOld > 360) {
      showToast(`⚠️ Editing expired: PO #${order.po_number || order.id} was created ${Math.floor(hoursOld / 24)} days ago. Edits are only permitted within 15 days (360h).`, 'error');
      return;
    }
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px' }}>
              <button
                type="button"
                onClick={() => setDeleteBlockedDialog({ isOpen: false, poNumber: '', message: '', linkedInvoices: [] })}
                style={{
                  padding: '9px 18px',
