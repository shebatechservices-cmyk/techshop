...
export default function useSettingsManager() {
  ...
  const handleRedeemCode = async (e) => {
    ...
    const cleanCode = (redemptionCode || '').trim();
    if (!cleanCode) {
      showToast('Please enter a valid redemption code', 'error');
      return;
    }
    ...
    try {
      ...
      if (res.ok && data.success) {
        setRedemptionResult({ success: true, message: data.message || 'Voucher code redeemed successfully!' });
        showToast(data.message || 'Redemption code applied successfully!', 'success');
        setRedemptionCode('');
        loadSettingsData();
      } else {
        setRedemptionResult({ success: false, message: data.message || 'Failed to redeem voucher code.' });
        showToast(data.message || 'Invalid redemption code', 'error');
      }
    } catch (err) {
      setRedemptionResult({ success: false, message: 'Failed to connect to the server.' });
