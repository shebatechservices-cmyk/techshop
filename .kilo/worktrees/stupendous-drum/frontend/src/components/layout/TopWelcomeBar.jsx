...
export default function TopWelcomeBar({ 
  ...
  return (
    <header className="top-header-wrapper">
      {/* 1. Left Section: Sidebar Toggle & Brand Header */}
        ...
        >
          ...
          <div className="top-header-brand-info">
            <div className="top-header-brand-title">
              <span className="brand-name">Sheba Technology</span>
      ...
      {/* 2. Center Section: Sleek Global Search Bar with Ctrl+K */}
      <div className="top-header-center">
        <GlobalSearchBar onNavigate={onNavigate} compact={true} />
      </div>

      {/* 3. Right Section: Grouped Shortcuts, Compact Clock, Utilities & User Profile */}
      <div className="top-header-right">
        ...
        <div style={{ position: 'relative' }} ref={calcRef}>
          ...
          {showCalc && (
            <div className="top-calc-popup">
              <div className="top-calc-display">
                <div className="top-calc-input">{calcInput || '0'}</div>
                <div className="top-calc-result-row">
                  <span className="top-calc-result">{calcResult || ''}</span>
                  {calcResult && (
                    <button
                      type="button"
                      className="top-calc-copy-btn"
              ...
              <div className="top-calc-grid">
                {['C', 'DEL', '/', '*', '7', '8', '9', '-', '4', '5', '6', '+', '1', '2', '3', '=', '0', '.', '00'].map((btn) => (
                  <button
                    key={btn}
                    type="button"
                    className={`top-calc-btn ${['C', 'DEL'].includes(btn) ? 'btn-danger' : ['/', '*', '-', '+', '='].includes(btn) ? 'btn-op' : ''}`}
