...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM users WHERE role_id = 4 OR email = 'tech1@shebatech.com.bd') THEN
        INSERT INTO users (name, email, password_hash, role_id)
        VALUES ('Al-Amin Technician', 'tech1@shebatech.com.bd', 'scrypt_or_hash_dummy', 4);
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM customers WHERE customer_type = 'technician') THEN
        ...
        VALUES ('Kamrul Hasan (Senior CCTV Tech)', '01711223344', 'Sarail Bazar, Brahmanbaria', 'technician', 'technician', 0);
    END IF;
END $$;
